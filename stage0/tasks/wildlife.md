...
```
4. В локальной папке приватного репозитория школы создайте подпапку `wildlife`, скопируйте в неё файлы вёрстки (`index.html`, `style.css`, папку `assets` с медиафайлами)
...
(в приватном репозитории школы для данного таска это требование не проверяется)
