...
- С момента открытия попапа, нажимаем на затемненную область вне попапа. Попап не закрывается. **-5**.
- При открытой вкладке create account нажимаем на log in (или наоборот, при открытой вкладке log in нажимаем на create account). При этом переключения не происходит.  **-5**.
...
2. **Карусель** сделана и реагирует на нажатие кнопок вправо/влево **+15**. Баллы снимаем, если:
- С момента загрузки, нажимаем несколько раз кнопку "влево", хотя бы 5 раз, смотрим чтобы карточки (все, или одна колонка) перелистывались постоянно. Если кнопка при этом перестает реагировать (переходит в неактивное состояние). **-5**.
- С момента загрузки, нажимаем несколько раз кнопку "вправо", хотя бы 5 раз, смотрим чтобы карточки (все, или одна колонка) перелистывались постоянно. Если кнопка при этом перестает реагировать (переходит в неактивное состояние). **-5**.
