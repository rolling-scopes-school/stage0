...
1. **Модальное окно (попап)** сделано и реагирует на нажатие кнопок Log In или Sign Up **+20**. Баллы снимаем, если нарушены следующие пункты:
- Закрытие попапа не происходит по нажатию на затемненную область вне попапа. **-5**.
- Не происходит переключение вкладок create account и log in. **-2**.
- Для вкладки create account на кнопке send нет валидации, или она не реагирует на заполнение полей Name, Email, Password или чекбокс "Agree...". **-2**.
- Для вкладки log in на кнопке send нет валидации, или она не реагирует на заполнение полей Email или Password. **-2**.
- Для вкладки log in при заполнении случаными значениями, кроме Email: `user@gmail.com` и Password: `useruser`, и нажатии кнопки send, не появляется текст с ошибкой (Incorrect email or password). **-2**. Рекомендуется, чтобы текст пропадал при вводе новых значений в одно из полей.
...
- Для вкладки log in, при заполнении полей Email: `user@gmail.com` и Password: `useruser`, и нажатии кнопки "send", не появляется иконка юзера с тултипом с именем пользователя `user` (или другим не пустым). Или не исчезнули кнопки Log In и Sign Up. **-2**.
...
2. **Карусель** сделана и реагирует на нажатие кнопок вправо/влево **+15**. Баллы снимаем, если нарушены следующие пункты:
...
- Иконки при увеличении карты не масштабируются. **-2**.
- Визуально, икноки сильно смещаются при масштабировании, больше чем на 50px, от начальной точки на карте **-2**.
...
- Иконки при уменьшении карты не масштабируются **-2**.
- Карта (или иконки животных) наезжает поверх элементов, которые не должны попадать под нее (хедер, футер, кнопки + и -) **-5**.
- При масштабировании, когда карта увеличена ине помещается в окно браузера, не работает drag'n'drop **-10**.


#### Zoos

1. **Карусель** в блоке Side cameras существует и реагирует на нажатие **+5**. Баллы снимаем, если:
- С момента загрузки, при нажитии на картинку видео в блоке Side cameras, этот элемент не стаонвится на место основного видео, или основное видео не становится второстепенным элементом в блоке Side cameras **-5**.

2. **Иконка ♡** в блоке Main camera существует и реагирует на нажатие **+5**. Баллы снимаем, если:
- Иконка ♡ не становится активной, или нет тереяет активность **-2**.
