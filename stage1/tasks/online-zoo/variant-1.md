...
1. Кнопки `Donate to volunteers` в макете оформлены некорректно. Надо либо поместить надпись целиком в кнопку, либо обрезать название до `donate`.
2. Анимацию изменения размера карточек животных, как указано на 1200px можно сделать по желанию. То же касается и маленьких видео в блоке `Side cameras`.
3. На 640px приведены отзывы в блоке `testimonials` с обрезанным 3-м элементом. Лучше уменьшить количество отзывов до 2х. Однако, если сделано как в макете, 3й отзыв, уезжающий за край экрана, баллы снижаться не будут.
4. Изменение элементов со сменой концепции лучше сделать через скрытие/отображение элементов. Речь о стрелочках в блоке `Look at this beautiful animals`, которые меняются с горизонтальных на вертикальную. Будет проще работать с ними в js.
